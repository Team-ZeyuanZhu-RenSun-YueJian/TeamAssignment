...
public class CarDaoImpl implements CarDao{
    ...
    @Override
    public ResultSet selectBySeat(String beginSeat, String endSeat) {
        try {
            ct = JdbcUtil.getConnection();
            String sql = "select * from cartable where seats >= ? and seats <= ?";
            ps = ct.prepareStatement(sql);
            ps.setString(1, beginSeat);
            ps.setString(2, endSeat);
            return ps.executeQuery();
        }catch(Exception e) {
            e.printStackTrace();
            return null;
    ...
    @Override
    public ResultSet selectByPrice(String beginPrice, String endPrice) {
        try {
            ct = JdbcUtil.getConnection();
            String sql = "select * from cartable where price >= ? and price <= ?";
            ps = ct.prepareStatement(sql);
            ps.setString(1, beginPrice);
